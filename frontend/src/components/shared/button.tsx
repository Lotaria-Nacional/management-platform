...
const Button = forwardRef<HTMLButtonElement, Props>(
  ({ className, children, variant = "red", ...props }, ref) => {
    const variantStyle: VariantStyle = {
      red: "bg-RED-700 !text-white hover:bg-RED-700/90",
    ...
    return (
      <button
        ...
        {...props}
        className={twMerge(
          `${variantStyle[variant]} flex items-center justify-center shadow-button text-center text-button font-medium gap-button p-button min-w-button h-button rounded-button cursor-pointer duration-200 ease-in-out transition-all`,
          className
        )}
      >
        {children}
