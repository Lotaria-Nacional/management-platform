        ...
        <TableBody>
          {TERMINAL_AND_POS_DATA.map((data) => (
            <TableRow key={data.agentId}>
              ...
              <TableCell>{data.actualBalance}</TableCell>
              <TableCell>
...
const TerminalAndPosTableCellStatus = ({ isActive }: { isActive: boolean }) => {
  return (
    <div
      className={`flex items-center justify-center gap-2 rounded-full p-2 h-8 min-w-20 max-w-24 ${
        isActive
